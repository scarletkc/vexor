...
class Messages:
    ...
    INIT_STEP_RUN_MODE = "1) How do you want to run Vexor?"
    INIT_OPTION_LOCAL = "A) Local - run embeddings on this machine"
    INIT_OPTION_REMOTE = "B) Remote - use an API provider"
    INIT_PROMPT_RUN_MODE = "Choose A/B"
    INIT_STEP_LOCAL_HARDWARE = "Local hardware:"
    INIT_OPTION_CPU = "A) CPU (default)"
    INIT_OPTION_CUDA = "B) CUDA (GPU)"
    ...
    INIT_OPTION_PROVIDER_OPENAI = "A) OpenAI (default)"
    INIT_OPTION_PROVIDER_GEMINI = "B) Gemini"
    INIT_OPTION_PROVIDER_CUSTOM = "C) Custom (OpenAI-compatible)"
    ...
    INIT_STEP_RERANK = "2) Enable rerank?"
    INIT_OPTION_RERANK_OFF = "1) Off (default)"
    INIT_OPTION_RERANK_BM25 = "2) BM25 (almost no extra latency)"
    INIT_OPTION_RERANK_FLASHRANK = "3) FlashRank (more accurate, slower)"
    INIT_OPTION_RERANK_REMOTE = "4) Remote (network-dependent)"
    ...
    INIT_STEP_ALIAS = "3) Shell alias"
    INIT_CONFIRM_ALIAS = "Create shell alias 'vx' for 'vexor'?"
    INIT_STEP_SKILLS = "4) Assistant skills"
    INIT_CONFIRM_SKILLS_INSTALL = "Install the vexor-cli skill for your assistant?"
    INIT_STEP_DOCTOR = "5) Verify setup"
    INIT_CONFIRM_DOCTOR = "Run `vexor doctor` now to verify your configuration?"
    INIT_STEP_SKILLS_TARGET = "Skill install target:"
    INIT_OPTION_SKILLS_AUTO = "A) auto (claude + codex)"
    INIT_OPTION_SKILLS_CLAUDE = "B) claude"
    INIT_OPTION_SKILLS_CODEX = "C) codex"
    INIT_OPTION_SKILLS_CUSTOM = "D) custom path"
    ...
    INIT_NEXT_STEPS_TITLE = "Next steps (copy/paste):"
