...
class Messages:
    ...
    ERROR_PROJECT_CONFIG_INVALID = "Invalid project config at {path}: {reason}"
    ERROR_PROJECT_CONFIG_SENSITIVE = (
        "Project config at {path} must not contain: {fields}. Credentials and "
        "endpoints must be configured globally or through environment variables."
    )
    ERROR_PROJECT_CONFIG_UNSUPPORTED = (
    ...
    INFO_CONFIG_SUMMARY = (
        ...
        "{flashrank_line}"
        "{remote_rerank_line}"
        "Local CUDA: {local_cuda} ({local_cuda_origin})\n"
        "Custom base URL: {base_url} ({base_url_origin})"
    )
    INFO_FLASHRANK_MODEL_SUMMARY = "FlashRank model: {value} ({origin})"
    INFO_REMOTE_RERANK_SUMMARY = "Remote rerank: {value} ({origin})"
    ...
    DOCTOR_PROJECT_CONFIG_EXISTS = "Project config file exists at {path}"
