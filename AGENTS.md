...
Follow PEP 8 with 4-space indentation and roughly 100-character lines. Use `snake_case` for modules/functions, `PascalCase` for classes, and lowercase imperative Typer command names. Type-annotate new Python code, keep CLI validation errors consistent with `typer.BadParameter` where appropriate, and route user-facing copy through `text.py` so CLI messaging stays centralized. Prefer structured rendering paths over ad hoc print strings, especially for Rich tables and porcelain output. Before writing new logic, look for an existing helper to reuse or extend (`utils.py`, `text.py`, the services layer) rather than duplicating an implementation.
